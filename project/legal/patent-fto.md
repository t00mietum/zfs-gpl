...
## Enumeration is kept private, on purpose

A preliminary live-vs-expired inventory (with dates) exists, but it is deliberately NOT published here. Publicly enumerating specific live patents alongside the features that would read on them can serve as willful-infringement evidence (treble damages) if a claim is ever brought. The working inventory therefore lives in private notes, and this file stays a framing placeholder until counsel produces a formal FTO opinion.

## To do

- 🔘 Engage a patent attorney for an FTO search.
- 🛠️ Enumerate live vs expired Sun/Oracle ZFS patents with real dates. Preliminary bibliographic inventory done (kept in private working notes, not public - see above); two gaps remain (RAID-Z parity, foundational COW grant) plus attorney-grade claim reads.
