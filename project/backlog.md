...
- 🛠️ Repo, license, provenance, clean-room process, workspace skeleton
	...
	- 🔘 Set up the spec repo (`zfs-gpl-spec`) and wire the dirty tree with its own role-constraining context
...
- 🔘 Stand up the spec pipeline: dirty-side analysis -> gatekeeper review -> spec repo, with the evidence trail

## Phase 2 - read the core format (from published spec)

- 🔘 Vdev labels: locate and validate all four, parse the name-value pairs
- 🔘 Uberblock: find the active uberblock (highest valid txg)
