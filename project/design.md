...
- Errors are values; never panic on disk input. `Result` at every decode boundary, a dedicated error enum, no `unwrap`/`expect`/indexing panics on bytes read from a device. `panic!` is only for violated internal invariants (programmer bugs). Corruption is an expected outcome, not an exception. Error-crate split: `thiserror` for the library crates (`zfsgpl-ondisk`, `zfsgpl-core`), `anyhow` at the application edge (`zfsgpl-cli`).
...
- Keep the happy path at minimum indentation. Guard-clause with early `return`/`continue`/`break`; `let-else` for extract-or-bail (`let Some(x) = opt else { return ... };`); `?` to propagate instead of nesting `match`/`if let`; let-chains where they read cleanly. No `else` after a `return`.
- Derive, do not hand-roll. `Debug` on every public type; `Clone`/`PartialEq`/etc. by derive where the semantics are the plain ones. Newtypes make invalid states unrepresentable where it is cheap.
- Lint bar is `clippy::pedantic`, warnings-as-errors; write code that passes it. Format with rustfmt as pinned (`hard_tabs = true` - the project pin overrides rustfmt's space default; do not "fix" it to spaces).
